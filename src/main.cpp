...
#if defined(NDEBUG)
# error "Lekan cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x213d667eadbb92068726b729c3ee9a65e9c3654bba1451214581caa225b3591f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lekan: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lekan Signed Message:\n";
...
{
    // Lekan: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 8400 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 8400000); // Lekan: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 24 * 60 * 60; // Lekan: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Lekan: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lekan: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lekan: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc5c38770297875a31a7d64ed0f7ad3283d4c47699c6ed1267e0859afb423585c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Worldometers.info 12/Apr/2020  Coronavirus  has Killed  Over 20,000 Americans";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 8400 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04862557b33ffed9625cc5b5c94d42fd417695f91b5706e5fa97b157e724de850ba935be4245c22b374012275bba66808ac1313573b0d4d655ea3cfbac417b3a69") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1586666635;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086538539;

        if (fTestNet)
        {
            block.nTime    = 1586666472;
            block.nNonce   = 385502990;
        }      
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x92f525f1731b2d235ddfd9d89686fb5134b9746baa7332e5f088dcba9a447223"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc9, 0xb2, 0xde }; // Lekan: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LekanMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LekanMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LekanMiner(CWallet *pwallet)
{
    printf("LekanMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lekan-miner");
    ...
    try { loop {
        ...
        printf("Running LekanMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LekanMiner, pwallet));
}
